...
class GetKeyword(Node):
    ...
    def get_keyword(self, request, response):  # 요청과 응답 객체를 받아야 함
        ...
        keyword, target_pos = self.extract_keyword(output_message)

        self.get_logger().warn(f"Detected tools: {keyword}, target positions: {target_pos}")

        # 응답 객체 설정
        response.success = True
        answer = keyword + target_pos
        # response.message = " ".join(keyword, target_pos)  # 감지된 키워드를 응답 메시지로 반환
