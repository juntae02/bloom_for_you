...
- **원인 분석** :  
  &nbsp;&nbsp;한정된 환경에서는 빠른 추론 속도와 낮은 리소스 사용량이 요구되기 때문에, 초기에는 경량 모델인 **YOLOv8n**을 사용했습니다.  
  하지만 YOLOv8n은 **화분과 꽃의 경계가 겹치는 경우**, 작은 객체인 꽃이 화분에 묻히는 현상으로 인해 **정확한 분리 탐지**에 한계가 있었습니다.  
...
|  👍 장점           | 낮은 리소스 사용, 경량 모델      | 높은 탐지 정확도, 높은 연산 효율         |  
|  👎 단점           | 작은 객체 탐지 한계, 연산 비효율성      | 많은 파라미터 수 , 큰 리소스 소모           |  
