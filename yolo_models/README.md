...
- **원인 분석** :  
  &nbsp;&nbsp;한정된 환경은 빠른 추론 속도와 낮은 리소스 사용량이 요구된다고 판단하여, 초기에는 경량 모델인 YOLOv8n를 사용하여 모델을 학습했습니다.  
  YOLOv8n 모델이 파라미터 수가 적고 가볍기 때문에 리소스 사용이 효율적이지만, 화분과 꽃의 경계가 겹치는 경우에 상대적으로 작은 객체인 꽃 탐지에는 한계가 있었습니다.
  하지만 해당 모델은 화분과 꽃이 경계가 겹치는 경우, 상대적으로 작은 객체인 꽃이 묻혀 정확한 분리 탐지가 어렵다는
...
- 📊 성능 개선 결과 :  
  ...
  이는 **작은 객체, 경계 겹침, 위치 변화** 등 다양한 조건에서도 탐지 정확도가 개선되었음을 보여줍니다.  
